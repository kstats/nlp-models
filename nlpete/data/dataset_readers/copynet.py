...
@DatasetReader.register("copynet_const")
class CopyNetDatasetReader(DatasetReader):
    """
    Read a tsv file containing paired sequences, and create a dataset suitable for a
    ``CopyNet`` model, or any model with a matching API.
    ...
    def __init__(self,
                 target_namespace: str,
                 source_tokenizer: Tokenizer = None,
                 const_tokenizer: Tokenizer = None,
                 target_tokenizer: Tokenizer = None,
                 source_token_indexers: Dict[str, TokenIndexer] = None,
                 target_token_indexers: Dict[str, TokenIndexer] = None,
                 lazy: bool = False) -> None:
        super().__init__(lazy)
        self._target_namespace = target_namespace
        self._const_namespace = const_tokenizer or WordTokenizer()
        self._source_tokenizer = source_tokenizer or WordTokenizer()
        self._target_tokenizer = target_tokenizer or self._source_tokenizer
        self._source_token_indexers = source_token_indexers or {"tokens": SingleIdTokenIndexer()}
        self._target_token_indexers = target_token_indexers or self._source_token_indexers
    ...
    @staticmethod
    def _read_line(line_num: int, line: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        line = line.strip("\n")
        if not line:
        ...
        if len(line_parts) != 3:
            raise ConfigurationError("Invalid line format: %s (line number %d)" % (line, line_num + 1))
        source_sequence, const_input, target_sequence = line_parts
        return source_sequence, const_input, target_sequence
    ...
    def _read(self, file_path):
        with open(cached_path(file_path), "r") as data_file:
            logger.info("Reading instances from lines in file at: %s", file_path)
            for line_num, line in enumerate(data_file):
                source_sequence, const_input, target_sequence = self._read_line(line_num, line)
                if not source_sequence:
                    continue
                yield self.text_to_instance(source_sequence, const_input, target_sequence)
    ...
    @overrides
    def text_to_instance(self, source_string: str, const_input: str, target_string: str = None) -> Instance:  # type: ignore
        """
        Turn raw source string and target string into an `Instance`.
        ...
        tokenized_source.insert(0, Token(START_SYMBOL))
        tokenized_source.append(Token(END_SYMBOL))
        const_tokens = [x for x in const_input.split(" ")]
        const_tokens.insert(0, "START")
        const_tokens.append("END")
        source_field = TextField(tokenized_source, self._source_token_indexers)
        ...
        meta_fields = {"source_tokens": [x.text for x in tokenized_source[1:-1]],
                       "const_tokens": [x for x in const_tokens]}
        fields_dict = {
                "source_tokens": source_field,
                "source_to_source": source_to_source_field,
                "source_to_target": source_to_target_field,
